1. **`requirements.txt`** — Add all required libraries such as LangGraph, Groq, Pinecone, Tavily, FastAPI, document loaders, etc.

2. **`.env`** — Configure API keys, Pinecone index/namespace, Groq model, embedding model, and other environment settings.
...
6. **`app/rag/vectorstore.py`** — Configure Huggingface embeddings, connect with Pinecone, store document vectors, and create the retriever.
